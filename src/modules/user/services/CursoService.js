...
export const useGetCurso = (id) => {
  ...
  const ONE_CURSO = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/curso/list-curso-secciones-tema?uuid=${id}`);
      return data;
    },
    enabled: !!id,
...
// ─── obtener one curso ──────────────────────────────────────────────────
export const getCursoService = (id) => api.get(`/curso/list-curso-secciones-tema?uuid=${id}`).then((res) => res.data);

// ? CREAR CALIFICACION
export const createCalificacionService = (data) => api.post(`/calificacion/create`, data).then((res) => res.data);
// ? GET CALIFICACION
export const getCalificacionesService = (data) =>
  api.get(`/calificacion/all?user_id=${data?.user_id}&curso_id=${data?.curso_id}`).then((res) => res.data);
